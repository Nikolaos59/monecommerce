...
if (!empty($_POST)) { // debug($_POST);
    ...
    if (!empty($_FILES['photo']['name'])) { // debug($_FILES);photo
...
if (isset($_GET['action']) && ($_GET['action'] == 'ajout' || $_GET['action'] == 'modification')) {
    if (isset($_GET['id_produit'])) {
        $resultat = executeRequete("SELECT * FROM produit WHERE id_produit=$_GET[id_produit]");
        $produit_actuel = $resultat = $resultat ->fetch_assoc();
    }
    echo '
<h1> Formulaire Produits </h1><br>
<form method="post" enctype="multipart/form-data" action="">
<input type="hidden" id="id=produit" name="id_produit" value="';
    ...
    elseif (!isset($produit_actuel) && !isset($_POST['public'])) echo 'checked'; echo'>Femme 
    <br><br>
